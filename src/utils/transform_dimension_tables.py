...
def transform_dim_counterparty():
   ...
   counterparty_df = json_to_pd_dataframe(most_recent_file,"counterparty","fscifa-raw-data")
   counterparty_df.drop(["created_at","last_updated","delivery_contact","commercial_contact"], axis=1, inplace=True)
   merge_location_to_counterparty_df = pd.merge(counterparty_df, 
                        transform_dim_location(), 
                        left_on="legal_address_id", 
                        right_on="location_id", how='left')
   merge_location_to_counterparty_df.drop(["location_id","legal_address_id"], axis=1, inplace=True)
   for column_name in list(merge_location_to_counterparty_df.columns):
      if column_name == "phone":
         merge_location_to_counterparty_df.rename(columns = {"phone" : f"counterparty_legal_phone_number"}, inplace = True)
      if column_name != "counterparty_id" and column_name != "counterparty_legal_name":
         merge_location_to_counterparty_df.rename(columns = {column_name : f"counterparty_legal_{column_name}"}, inplace = True)
...
def transform_dim_currency():
   ...
   currency_names = [CurrencyCodes.get_currency_name(currency_code=code) for code in currency_df["currency_code"]]
   currency_df["currency_name"] = currency_names
   return currency_df
