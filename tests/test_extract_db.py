...
class TestExtractDB:
    ...
    def test_extract_db_returns_dictionary_with_expected_key_and_list_value(self,mock_connect_to_db):
        mock_conn = Mock()
    ...
    @pytest.mark.it("Testing extract db with empty table returns dictionary with empty list")
    @patch("src.utils.extract_db.connect_to_db")
    ...
    def test_extract_db_returns_expected_values(self,mock_connect_to_db):
        mock_conn = Mock()
        mock_conn.run.return_value = [[1, 'person', 'one', 2, 'person1@terrifictotes.com', datetime.datetime(2022, 11, 3, 14, 20, 51, 563000), 
                                    datetime.datetime(2022, 11, 3, 14, 20, 51, 563000)],
                                    [2, 'person', 'two', 6, 'person2@terrifictotes.com', datetime.datetime(2022, 11, 3, 14, 20, 51, 563000), datetime.datetime(2022, 11, 3, 14, 20, 51, 563000)]]
        ...
        expected = {"staff":[{'staff_id': 1, 'first_name': 'person', 'last_name': 'one', 'department_id': 2, 'email_address': 'person1@terrifictotes.com', 'created_at': datetime.datetime(2022, 11, 3, 14, 20, 51, 563000), 'last_updated': datetime.datetime(2022, 11, 3, 14, 20, 51, 563000)}, {'staff_id': 2, 'first_name': 'person', 'last_name': 'two', 'department_id': 6, 'email_address': 'person2@terrifictotes.com', 'created_at': datetime.datetime(2022, 11, 3, 14, 20, 51, 563000), 'last_updated': datetime.datetime(2022, 11, 3, 14, 20, 51, 563000)}]}
        result = extract_db("staff")
        assert expected == result