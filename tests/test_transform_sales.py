...
class TestMakeFactSalesOrderTable():
    @pytest.mark.it("Tests that the make_fact_sales_order_table function makes a fact table correctly")
...
class TestMakeDimDate():
    @pytest.mark.it("Tests that the make_dim_date function outputs the expected dim table")
    def test_make_dim_date_creates_correct_dim_table(self):
        ...
        expected_dim_table = pd.DataFrame({
            ...
            'day_of_week': [4, 2, 5, 4, 5, 0],
            'day_name': ['Friday', 'Wednesday', 'Saturday', 'Friday', 'Saturday', 'Monday'],
            'month_name': ['May', 'June', 'May', 'May', 'February', 'February'],
            'quarter': [2, 2, 2, 2, 1, 1]
        })
        pd.testing.assert_frame_equal(dim_table.sort_index(axis=1), expected_dim_table.sort_index(axis=1), check_like = True)
