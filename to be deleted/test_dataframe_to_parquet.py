...
@patch("src.utils.dataframe_to_parquet.datetime.now")
def test_df_to_parquet_successful_creation_asserting_by_path(
    mock_datetime, mock_to_parquet, mock_df
):
    fixed_time = datetime(2025, 6, 4, 10, 13, 0)
    mock_datetime.return_value = fixed_time
    table_name = "test-staff"
...
def test_df_to_parquet_successful_creation_by_reading_parquet_file(
    mock_datetime, mock_df
):
    fixed_time = datetime(2025, 6, 4, 10, 13, 0)
